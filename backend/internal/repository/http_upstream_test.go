...
func TestHTTPUpstreamForcedPublicIPValidationRejectsLoopbackWhenGlobalAllowlistDisabled(t *testing.T) {
	svc := &httpUpstreamService{}
	req, err := http.NewRequestWithContext(
		urlvalidator.WithPublicIPValidation(context.Background()),
		http.MethodGet,
		"https://127.0.0.1/v1/responses",
		nil,
	)
	require.NoError(t, err)
	require.ErrorContains(t, svc.validateRequestHost(req), "not allowed")
	require.ErrorContains(t, svc.redirectChecker(req, nil), "not allowed")
}

func TestHTTPUpstreamForcedPublicIPValidationRejectsHTTPRedirect(t *testing.T) {
	svc := &httpUpstreamService{}
	req, err := http.NewRequestWithContext(
		urlvalidator.WithPublicIPValidation(context.Background()),
		http.MethodGet,
	...
	require.NoError(t, err)
	require.ErrorContains(t, svc.redirectChecker(req, nil), "must use https")
}
