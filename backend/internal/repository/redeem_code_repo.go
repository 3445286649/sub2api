...
func (r *redeemCodeRepository) Create(ctx context.Context, code *service.RedeemCode) error {
	created, err := r.client.RedeemCode.Create().
		...
		SetType(code.Type).
		SetValue(code.Value).
		SetAffiliateRebateBaseAmount(code.AffiliateRebateBaseAmount).
		...
		SetNillableCampaignID(code.CampaignID).
		Save(ctx)
	if err == nil {
		code.ID = created.ID
...
func (r *redeemCodeRepository) CreateBatch(ctx context.Context, codes []service.RedeemCode) error {
	...
	for i := range codes {
		...
		b := r.client.RedeemCode.Create().
			...
			SetType(c.Type).
			SetValue(c.Value).
			SetAffiliateRebateBaseAmount(c.AffiliateRebateBaseAmount).
			SetStatus(c.Status).
			SetNotes(c.Notes).
			...
			SetNillableCampaignID(c.CampaignID)
