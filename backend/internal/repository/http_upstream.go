...
func (s *httpUpstreamService) validateRequestHost(req *http.Request) error {
	forcePublicValidation := req != nil && urlvalidator.RequiresPublicIPValidation(req.Context())
	publicHostsOnly := req != nil && service.HTTPUpstreamPublicHostsOnly(req.Context())
	if !forcePublicValidation && !publicHostsOnly && !s.shouldValidateResolvedIP() {
		return nil
	}
	if req == nil || req.URL == nil {
		return errors.New("request url is nil")
	}
	if forcePublicValidation && !strings.EqualFold(strings.TrimSpace(req.URL.Scheme), "https") {
		return errors.New("request url must use https")
	}
