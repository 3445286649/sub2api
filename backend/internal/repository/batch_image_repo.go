...
func (r *batchImageRepository) ListBatchImageJobsDueForInputCleanup(ctx context.Context, cutoff time.Time, limit int) ([]*service.BatchImageJob, error) {
	...
	now := time.Now().UTC()
	leaseUntil := now.Add(10 * time.Minute).Format("2006-01-02T15:04:05.000000000Z07:00")
...
WITH candidates AS (
    ...
    WHERE input_deleted_at IS NULL
      ...
      AND (
          last_error_code IS DISTINCT FROM 'BATCH_IMAGE_CLEANUP_CLAIM_INPUT'
          OR last_error_message IS NULL
          OR last_error_message < $2
      )
    ORDER BY id ASC
    LIMIT $3
...
SET last_error_code = 'BATCH_IMAGE_CLEANUP_CLAIM_INPUT',
    last_error_message = $4,
    updated_at = $5,
    version = version + 1
FROM candidates
WHERE j.id = candidates.id
RETURNING `+batchImageJobColumns, cutoff, now.Format("2006-01-02T15:04:05.000000000Z07:00"), limit, leaseUntil, now)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
WITH candidates AS (
    ...
    WHERE output_deleted_at IS NULL
      ...
      AND (
          last_error_code IS DISTINCT FROM 'BATCH_IMAGE_CLEANUP_CLAIM_OUTPUT'
          OR last_error_message IS NULL
          OR last_error_message < $2
      )
...
UPDATE batch_image_jobs AS j
SET last_error_code = 'BATCH_IMAGE_CLEANUP_CLAIM_OUTPUT',
    last_error_message = $4,
    updated_at = $5,
    version = version + 1
FROM candidates
WHERE j.id = candidates.id
RETURNING `+batchImageJobColumns, now, now.Format("2006-01-02T15:04:05.000000000Z07:00"), limit, leaseUntil, now)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
SET input_deleted_at = CASE WHEN input_deleted_at IS NULL THEN $2 ELSE input_deleted_at END,
    last_error_code = CASE WHEN last_error_code = 'BATCH_IMAGE_CLEANUP_CLAIM_INPUT' THEN NULL ELSE last_error_code END,
    last_error_message = CASE WHEN last_error_code = 'BATCH_IMAGE_CLEANUP_CLAIM_INPUT' THEN NULL ELSE last_error_message END,
    updated_at = $2,
    version = version + 1
