	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "recharge_ratio", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot"}).
					AddRow(tt.identityUnchanged, false, true, tt.databaseEnabled, nil, nil, tt.databaseSnapshot, nil, nil, nil))
...
func TestLockAndMergeAccountProbeExtraInvalidatesSnapshotWhenRechargeRatioChanges(t *testing.T) {
	for _, tt := range []struct {
		...
		{name: "changed ratio invalidates snapshot", currentRatio: []byte(`10`), explicit: 20, wantSnapshot: false},
		{name: "same ratio preserves snapshot", currentRatio: []byte(`10`), explicit: 10, wantSnapshot: true},
		{name: "explicit default matches missing ratio", currentRatio: nil, explicit: 1, wantSnapshot: true},
	} {
		t.Run(tt.name, func(t *testing.T) {
			db, mock, err := sqlmock.New()
			require.NoError(t, err)
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				WithArgs(int64(32), service.PlatformOpenAI, service.AccountTypeAPIKey, `{"api_key":"sk-test"}`, nil).
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "recharge_ratio", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot"}).
					AddRow(true, false, true, []byte(`true`), []byte(`true`), tt.currentRatio, []byte(`{"status":"ok"}`), nil, nil, nil))

			account := &service.Account{
				ID: 32, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey,
				Credentials: map[string]any{"api_key": "sk-test"},
			}
			got, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil, &tt.explicit)
			require.NoError(t, err)
			require.Equal(t, tt.explicit, got[service.UpstreamBillingRechargeRatioExtraKey])
			require.Equal(t, tt.wantSnapshot, got[service.UpstreamBillingProbeExtraKey] != nil)
			require.NoError(t, mock.ExpectationsWereMet())
		})
	}
}
...
func TestUpdateWithAccountBillingSettingsRollsBackWhenOutboxFails(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		WithArgs(int64(27), service.PlatformOpenAI, service.AccountTypeAPIKey, `{"api_key":"sk-test"}`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "recharge_ratio", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot"}).
			AddRow(true, false, true, []byte(`true`), []byte(`true`), nil, []byte(`{"status":"ok"}`), nil, nil, nil))
