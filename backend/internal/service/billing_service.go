...
func (s *BillingService) initFallbackPricing() {
	...
	// GPT-5.6 业务默认价；渠道模型定价仍可按分组覆盖。
	for model, spec := range gpt56BusinessPricing {
		s.fallbackPrices[model] = spec.modelPricing()
	}
...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	// GPT-5.6 使用固定业务默认价，避免动态价格数据缺失或陈旧时回退到 GPT-5.4。
	// 渠道覆盖在 ModelPricingResolver / GetModelPricingWithChannel 中后置应用。
	if spec, ok := getGPT56PricingSpec(model); ok {
		return s.applyModelSpecificPricingPolicy(model, spec.modelPricing()), nil
	}
