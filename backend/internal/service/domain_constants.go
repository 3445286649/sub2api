...
const (
	...
	// SettingKeyRechargeStorefrontChannels stores the validated channel list as JSON.
	SettingKeyRechargeStorefrontChannels  = "recharge_storefront_channels"
	...
	SettingKeyModelRadarEnabled           = "model_radar_enabled"           // 是否展示模型雷达入口
	SettingKeyModelRadarConfig            = "model_radar_config"            // 模型雷达运行配置（JSON，含加密测试 Key）
	...
	SettingKeyGatewaySchedulingWeights    = "gateway_scheduling_weights"    // 账号调度同层加权权重（JSON，四项总和 100）
	...
	// SettingKeyChannelMonitorMode selects exclusive implementation: v1 active probes, v2 passive aggregation.
	...
	// SettingKeyChannelMonitorHideThroughput hides RPM/TPM rates from user-facing monitor APIs.
