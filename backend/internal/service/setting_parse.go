...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyUsageRebateEnabled:            "false",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result.UsageRebateEnabled = settings[SettingKeyUsageRebateEnabled] == "true"
	...
	applyLocalFeatureSettingDefaults(result, settings, s.cfg)
...
func applyLocalFeatureSettingDefaults(result *SystemSettings, settings map[string]string, cfg *config.Config) {
	if result == nil {
		return
	}
	result.RechargeStorefrontEnabled = settings[SettingKeyRechargeStorefrontEnabled] == "true"
	result.RechargeStorefrontButtonText = strings.TrimSpace(settings[SettingKeyRechargeStorefrontText])
	if result.RechargeStorefrontButtonText == "" {
		result.RechargeStorefrontButtonText = "充值商城"
	}
	...
	if result.SupportGroupButtonText == "" {
		result.SupportGroupButtonText = "售后群"
	}
	result.SupportGroupTitle = strings.TrimSpace(settings[SettingKeySupportGroupTitle])
	if result.SupportGroupTitle == "" {
		result.SupportGroupTitle = "售后服务群"
	}
	result.SupportGroupDescription = strings.TrimSpace(settings[SettingKeySupportGroupDescription])
	result.SupportGroupQRCodeURL = strings.TrimSpace(settings[SettingKeySupportGroupQRCodeURL])
	result.SupportGroupLinkURL = strings.TrimSpace(settings[SettingKeySupportGroupLinkURL])
	result.SupportTicketsEnabled = !isFalseSettingValue(settings[SettingKeySupportTicketsEnabled])

	result.PixmoStudioEnabled = settings[SettingKeyPixmoStudioEnabled] == "true"
	result.PixmoStudioButtonText = strings.TrimSpace(settings[SettingKeyPixmoStudioButtonText])
	if result.PixmoStudioButtonText == "" {
		result.PixmoStudioButtonText = "Pixmo 生图"
	}
	result.PixmoStudioURL = strings.TrimSpace(settings[SettingKeyPixmoStudioURL])
	if result.PixmoStudioURL == "" {
		result.PixmoStudioURL = "https://pixmo.loucer.cn/"
	}
