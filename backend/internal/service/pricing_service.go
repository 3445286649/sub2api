...
func (s *PricingService) GetModelPricing(modelName string) *LiteLLMModelPricing {
	...
	// GPT-5.6 使用代码中的业务默认价，避免动态数据让展示价格与真实扣费不一致。
	if spec, ok := getGPT56PricingSpec(modelLower); ok {
		return spec.liteLLMPricing()
	}
...
func (s *PricingService) matchOpenAIModel(model string) *LiteLLMModelPricing {
	...
	// GPT-5.6 使用业务默认价，渠道模型定价可在计费 resolver 中继续覆盖。
	if spec, ok := getGPT56PricingSpec(model); ok {
		logger.With(zap.String("component", "service.pricing")).
			Info(fmt.Sprintf("[Pricing] OpenAI fallback matched %s -> %s", model, normalizeKnownOpenAICodexModel(model)+"(static)"))
		return spec.liteLLMPricing()
	}
