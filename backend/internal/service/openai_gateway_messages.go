...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.IsGrokAnthropicMessagesAPIKey() {
		return s.forwardGrokAnthropicMessagesAPIKeyPassthrough(ctx, c, account, body, defaultMappedModel)
	}
	...
	if account.Platform == PlatformGrok && account.Type == AccountTypeAPIKey && account.GetGrokUpstreamProtocol() == GrokUpstreamProtocolOpenAIChatCompletions {
