...
var providerSensitiveConfigFields = map[string]map[string]struct{}{
	...
	payment.TypeUSDTBSC:   {"bscscanapikey": {}, "apikey": {}},
}
...
var validProviderKeys = map[string]bool{
	payment.TypeEasyPay: true, payment.TypeAlipay: true, payment.TypeWxpay: true, payment.TypeStripe: true, payment.TypeAirwallex: true, payment.TypeUSDTBSC: true,
}
...
// decryptConfig parses a stored provider config. New records are AES-256-GCM
// ciphertext ("iv:authTag:ciphertext"); plaintext JSON is accepted as a legacy
// read-path shim so existing deployments can be migrated by re-saving configs.
...
func (s *PaymentConfigService) encryptConfig(cfg map[string]string) (string, error) {
	...
	if len(s.encryptionKey) != payment.AES256KeySize {
		return "", infraerrors.InternalServer("PAYMENT_CONFIG_ENCRYPTION_UNAVAILABLE", "payment config encryption key is not configured")
	}
