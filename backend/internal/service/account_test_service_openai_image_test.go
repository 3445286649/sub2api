...
func TestAccountTestService_OpenAIImageAPIKeyRejectsResponseWithoutImagePayload(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{"data":[{"revised_prompt":"![image]"}]}`)),
		},
	}
	svc := &AccountTestService{
		httpUpstream: upstream,
	...
	err := svc.testOpenAIImageAPIKey(c, context.Background(), account, "gemini-2.5-flash-image", "draw a cat")
	require.ErrorContains(t, err, "No image payload returned from API")
	require.Contains(t, rec.Body.String(), `"type":"error"`)
	require.NotContains(t, rec.Body.String(), `"success":true`)
}

func TestAccountTestService_OpenAIAPIKeyGeminiImageUsesImagesGenerationsAndEmitsURL(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/54/test", nil)
	...
	upstream := &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Body: io.NopCloser(strings.NewReader(`{"data":[{"url":"https://image-upstream.example/generated/cat.png"}]}`)),
		},
	}
	account := Account{
		ID:       54,
	...
	require.NotNil(t, upstream.lastReq)
	require.Equal(t, "https://image-upstream.example/v1/images/generations", upstream.lastReq.URL.String())
	require.Equal(t, "gemini-2.5-flash-image", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, "draw a tiny orange cat astronaut", gjson.GetBytes(upstream.lastBody, "prompt").String())
	...
	require.Contains(t, rec.Body.String(), `"image_url":"https://image-upstream.example/generated/cat.png"`)
	require.Contains(t, rec.Body.String(), `"success":true`)
}

func TestAccountTestService_OpenAIOAuthRejectsGeminiImageBeforeUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	...
	err := svc.TestAccountConnection(c, account.ID, "gemini-2.5-flash-image", "draw a cat", AccountTestModeDefault)
	require.ErrorContains(t, err, "Gemini image models require an OpenAI-compatible API key account")
