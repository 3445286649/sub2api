...
type batchLimitsStepUpCacheStub struct {
	service.TotpCache
	granted bool
}

func (s *batchLimitsStepUpCacheStub) HasStepUpGrant(context.Context, int64, string) (bool, error) {
	return s.granted, nil
}

type batchLimitsStepUpUserRepoStub struct {
	service.UserRepository
	user *service.User
}

func (s *batchLimitsStepUpUserRepoStub) GetByID(context.Context, int64) (*service.User, error) {
	return s.user, nil
}

func (s *batchLimitsStepUpUserRepoStub) GetUserAvatar(context.Context, int64) (*service.UserAvatar, error) {
	return nil, nil
}
...
func setupBatchLimitsRouter(serviceStub service.AdminService) *gin.Engine {
	return setupBatchLimitsRouterWithStepUp(serviceStub, nil, nil)
}

func setupBatchLimitsRouterWithStepUp(
	serviceStub service.AdminService,
	totpService *service.TotpService,
	userService *service.UserService,
	authSetup ...gin.HandlerFunc,
) *gin.Engine {
...
func TestUserHandlerBatchUpdateLimitsAllUsesEveryListedUser(t *testing.T) {
	...
	userRepo := &batchLimitsStepUpUserRepoStub{user: &service.User{ID: 1, TotpEnabled: true}}
	userService := service.NewUserService(userRepo, nil, nil, nil)
	totpService := service.NewTotpService(nil, nil, &batchLimitsStepUpCacheStub{granted: true}, nil, nil, nil)
	router := setupBatchLimitsRouterWithStepUp(serviceStub, totpService, userService, func(c *gin.Context) {
		c.Set("auth_method", "jwt")
		c.Set(string(middleware.ContextKeyUser), middleware.AuthSubject{UserID: 1})
		c.Set(middleware.ContextKeySessionID, "session-1")
		c.Next()
	})
	recorder := postBatchLimits(t, router, []byte(`{"all":true,"user_ids":[999],"rpm_limit":0}`))
...
func TestUserHandlerBatchUpdateLimitsAllRejectsAdminAPIKey(t *testing.T) {
	serviceStub := &batchLimitsAdminServiceStub{stubAdminService: newStubAdminService()}
	router := setupBatchLimitsRouterWithStepUp(serviceStub, nil, nil, func(c *gin.Context) {
		c.Set("auth_method", service.AuditAuthMethodAdminAPIKey)
		c.Next()
	})
	recorder := postBatchLimits(t, router, []byte(`{"all":true,"rpm_limit":10}`))
