...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	var lastFailoverErr *service.UpstreamFailoverError
	var oauth429FailoverState service.OpenAIOAuth429FailoverState
	handleWSFailover := func(account *service.Account, failoverErr *service.UpstreamFailoverError, healthFailureAlreadyRecorded bool) bool {
		if ctx.Err() != nil {
			return false
		}
		if !healthFailureAlreadyRecorded {
	...
	for {
		...
		if err != nil {
			...
			var failoverErr *service.UpstreamFailoverError
			if errors.As(err, &failoverErr) {
				if handleWSFailover(account, failoverErr, false) {
					continue
				}
				return
			}
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			var failoverErr *service.UpstreamFailoverError
			if errors.As(err, &failoverErr) {
				if handleWSFailover(account, failoverErr, wsTurnFailureRecorded.Load()) {
					continue
				}
				return
			}
