...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	supportTicketMessageRepository := repository.NewSupportTicketMessageRepository(client)
	supportService := service.NewSupportService(supportTicketRepository, supportTicketMessageRepository, userRepository)
	supportHandler := handler.NewSupportHandler(supportService, settingService)
	...
	accountUpstreamBalanceService := service.ProvideAccountUpstreamBalanceService(accountUpstreamBalanceRepository, accountRepository)
	accountHandler := handler.ProvideAdminAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, accountHealthService, accountUpstreamBalanceService, grokQuotaService)
	accountSchedulingHoldRepository := repository.NewAccountSchedulingHoldRepository(db, accountRepository, schedulerCache)
	accountSchedulingHoldService := service.NewAccountSchedulingHoldService(accountSchedulingHoldRepository)
	accountSchedulingHoldHandler := admin.NewAccountSchedulingHoldHandler(accountSchedulingHoldService)
	...
	pointsShopService := service.NewPointsShopService(db, billingCacheService)
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, pointsShopService, notificationEmailService)
	...
	pointsShopHandler := admin.NewPointsShopHandler(pointsShopService)
	dailyCheckinService := service.NewDailyCheckinService(db, billingCacheService)
	dailyCheckinHandler := admin.NewDailyCheckinHandler(dailyCheckinService)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	acquisitionRepository := repository.NewAcquisitionRepository(db)
	acquisitionService := service.ProvideAcquisitionService(acquisitionRepository, settingService, apiKeyAuthCacheInvalidator, billingCacheService)
	acquisitionHandler := admin.NewAcquisitionHandler(acquisitionService)
	usageRebateRepository := repository.NewUsageRebateRepository(db)
	usageRebateService := service.ProvideUsageRebateService(usageRebateRepository, settingService, apiKeyAuthCacheInvalidator, billingCacheService)
	usageRebateHandler := admin.NewUsageRebateHandler(usageRebateService)
	modelRadarRepository := repository.NewModelRadarRepository(db)
	modelRadarService := service.ProvideModelRadarService(modelRadarRepository, settingRepository, settingService, secretEncryptor, apiKeyService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, accountSchedulingHoldHandler, adminAnnouncementHandler, adminSupportHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, pointsShopHandler, dailyCheckinHandler, affiliateHandler, acquisitionHandler, usageRebateHandler, modelRadarHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator, channelMonitorService)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, channelMonitorService)
	...
	handlerPointsShopHandler := handler.NewPointsShopHandler(pointsShopService)
	handlerDailyCheckinHandler := handler.NewDailyCheckinHandler(dailyCheckinService)
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentService, registry)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	handlerAcquisitionHandler := handler.NewAcquisitionHandler(acquisitionService)
	handlerUsageRebateHandler := handler.NewUsageRebateHandler(usageRebateService)
	handlerModelRadarHandler := handler.NewModelRadarHandler(modelRadarService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, supportHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, handlerPointsShopHandler, handlerDailyCheckinHandler, paymentWebhookHandler, availableChannelHandler, handlerAcquisitionHandler, handlerUsageRebateHandler, handlerModelRadarHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	accountSchedulingHoldExpiryRunner := service.ProvideAccountSchedulingHoldExpiryRunner(accountSchedulingHoldService)
