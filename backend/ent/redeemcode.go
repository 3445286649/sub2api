...
func (_m *RedeemCode) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case redeemcode.FieldAffiliateRebateBaseAmount:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field affiliate_rebate_base_amount", values[i])
			} else if value.Valid {
				_m.AffiliateRebateBaseAmount = value.Float64
			}
...
func (_m *RedeemCode) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("affiliate_rebate_base_amount=")
	builder.WriteString(fmt.Sprintf("%v", _m.AffiliateRebateBaseAmount))
	builder.WriteString(", ")
