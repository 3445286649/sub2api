...
func (_q *SupportTicketMessageQuery) QuerySender() *UserQuery {
...
func (_q *SupportTicketMessageQuery) WithSender(opts ...func(*UserQuery)) *SupportTicketMessageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SupportTicketMessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SupportTicketMessage, error) {
	...
	if query := _q.withSender; query != nil {
		if err := _q.loadSender(ctx, query, nodes, nil,
			func(n *SupportTicketMessage, e *User) { n.Edges.Sender = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SupportTicketMessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*SupportTicketMessage, init func(*SupportTicketMessage), assign func(*SupportTicketMessage, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*SupportTicketMessage)
	for i := range nodes {
		fk := nodes[i].SenderID
...
func (_q *SupportTicketMessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withSender != nil {
			_spec.Node.AddColumnOnce(supportticketmessage.FieldSenderID)
		}
	}
