        ...
        <div v-if="!authStore.isSimpleMode" class="sidebar-section">
          ...
          <template v-for="item in personalNavItems" :key="item.path">
            <template v-if="item.children?.length">
              ...
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="mb-1 ml-4 border-l border-gray-200 pl-2 dark:border-dark-600">
                <router-link v-for="child in item.children" :key="child.path" :to="child.path" class="sidebar-link mb-0.5 py-1.5 text-sm" :class="{ 'sidebar-link-active': isActive(child.path) }" @click="handleMenuItemClick(child.path)">
                  <component :is="child.icon" class="h-4 w-4 flex-shrink-0" />
                  <span>{{ child.label }}</span>
                </router-link>
              </div>
            ...
            >
              ...
              <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
                <span class="min-w-0 truncate">{{ item.label }}</span>
                <span v-if="item.badge && !sidebarCollapsed" class="ml-auto rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-white">{{ item.badge > 99 ? '99+' : item.badge }}</span>
              </span>
            </router-link>
          </template>
        </div>
      ...
      <template v-else-if="!appStore.backendModeEnabled">
        <div class="sidebar-section">
          <template v-for="item in userNavItems" :key="item.path">
            <template v-if="item.children?.length">
              <button type="button" class="sidebar-link mb-1 w-full" :class="{ 'sidebar-link-active': isGroupActive(item) && !isGroupExpanded(item), 'sidebar-link-collapsed': sidebarCollapsed }" :title="sidebarCollapsed ? item.label : undefined" @click="handleGroupClick(item)">
                <component :is="item.icon" class="h-5 w-5 flex-shrink-0" />
                <span class="sidebar-label sidebar-label-flex" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }" :aria-hidden="sidebarCollapsed ? 'true' : 'false'">
                  <span class="min-w-0 truncate">{{ item.label }}</span>
                  <ChevronDownIcon class="h-4 w-4 flex-shrink-0 transition-transform duration-200" :class="isGroupExpanded(item) ? 'rotate-180' : ''" />
...
const flagRechargeStorefront = () => {
  return resolveRechargeStorefrontChannels(appStore.cachedPublicSettings).length > 0
}

// buildSelfNavItems 构造用户自己的导航项（用户端主菜单和管理员的"我的账户"子菜单共享这组声明）。
// withDashboard=true 时包含仪表盘（用户端），false 时不含（管理员的个人区已经有独立仪表盘入口）。
//
// 条目顺序：密钥 → 用量 → 可用渠道 → 渠道状态 → 订阅/支付 → 备用充值 → 兑换/资料。
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    {
      path: '/backup-recharge',
      label: t('nav.backupRecharge'),
      icon: CreditCardIcon,
      hideInSimpleMode: true,
      expandOnly: true,
      featureFlag: flagRechargeStorefront,
      children: resolveRechargeStorefrontChannels(appStore.cachedPublicSettings).map((channel) => ({
        path: `/backup-recharge/${channel.id}`,
        label: channel.name,
        icon: CreditCardIcon,
      })),
    },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/daily-checkin', label: t('nav.dailyCheckin'), icon: GiftIcon, hideInSimpleMode: true },
    ...
    },
    {
      ...
      children: [
        ...
        { path: '/admin/orders/points', label: t('nav.pointsShop'), icon: GiftIcon },
      ],
    },
