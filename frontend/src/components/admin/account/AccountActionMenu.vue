<template>
  <Teleport to="body">
    <div v-if="show && position">
      ...
      <div
        class="action-menu-content fixed z-[9999] w-52 overflow-hidden rounded-xl bg-white text-gray-700 shadow-lg ring-1 ring-black/5 dark:bg-dark-800 dark:text-gray-200"
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'health-detail', 'health-probe', 'duplicate', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy', 'create-spark-shadow'])
const { t } = useI18n()
