export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      toolActions: '工具',
      healthOverview: '健康总览',
      healthOverviewFailed: '健康总览加载失败',
      healthAccountCount: '{count} 个账号',
      upstreamBalanceValue: '余额 {value}',
      upstreamBalanceUnitUpstream: '{value} 上游额度',
      ...
      upstreamBalanceRefresh: '刷新上游余额',
      upstreamBalanceRefreshSuccess: '上游余额已刷新',
      upstreamBalanceRefreshFailed: '上游余额刷新失败',
      upstreamBalanceRepresentative: '代表账号 #{id}',
      upstreamBalanceCheckedAt: '查询时间：{time}',
      upstreamBalanceEndpoint: '接口：{endpoint}',
      upstreamBalanceUnitHint: 'new-api 兼容接口返回的额度单位按上游站点展示配置为准',
      upstreamBalanceHTTPStatus: 'HTTP 状态：{status}',
      healthInsufficientGroups: '可用账号不足：{groups}',
      riskSummary: '风险摘要',
      riskGroupNoAvailable: '分组 {group} 无可用账号',
      riskGroupSingleAvailable: '分组 {group} 只剩 1 个可用账号',
      riskUrlAllIsolated: '该 URL 下 {count} 个账号全部不可用',
      ...
      healthDetailLoadFailed: '健康详情加载失败',
      healthProbeSuccess: '健康探活完成',
      healthProbeFailed: '健康探活失败',
      probeSettings: '探活设置',
      probeSettingsHint: '关闭后后台不会自动探活恢复；手动健康探活仍可使用。间隔留空或 0 使用默认退避。健康巡检会产生真实上游请求成本。',
      probeEnabled: '后台探活',
      probeInterval: '探活间隔（分钟）',
      probeIntervalCustom: '自定义间隔（分钟）',
      probeIntervalPlaceholder: '默认退避',
      probeIntervalInvalid: '探活间隔必须是 0 或正整数分钟',
      healthProbeModel: '探活模型',
      healthProbeModelAuto: '自动默认模型',
      healthProbeModelHint: '留空使用平台默认测试模型；建议为国模、Kiro、ClaudeCode 选择真实可用模型。',
      healthProbeSyncModels: '同步上游模型',
      healthProbeSyncModelsSuccess: '上游模型已同步',
      healthProbeSyncModelsFailed: '上游模型同步失败',
      recoveryPriority: '恢复优先级',
      recoveryPriorityDefault: '默认退避',
      recoveryPriorityFast: '快速 3 分钟',
      recoveryPriorityStandard: '标准 5 分钟',
      recoveryPriorityConservative: '保守 10 分钟',
      healthyProbeEnabled: '健康账号低频巡检',
      healthyProbeHint: '默认关闭；仅对可调度账号生效，失败后进入扣分、降级和快速恢复链路。',
      healthyProbeInterval: '巡检间隔',
      healthyProbeIntervalOption: '{hours} 小时',
      healthyProbeIntervalOptionMinutes: '{minutes} 分钟',
      healthyProbeIntervalOptionHours: '{hours} 小时',
      healthyProbeIntervalHint: '设置为 1 分钟会显著增加上游探活请求，建议只给少量核心账号或测试场景使用。',
      healthyProbeIntervalInvalid: '健康巡检间隔必须是正整数分钟',
      probeSettingsSaved: '探活设置已保存',
      probeSettingsSaveFailed: '探活设置保存失败',
      overviewColumns: {
        account: '账号',
        key: 'Key 指纹',
        health: '健康度',
      ...
      healthEvents: '健康事件流',
      healthEventsHint: '记录真实请求、后台探活、手动探活、设置变更触发的扣分、恢复和隔离。',
      healthEventsEmpty: '暂无健康事件',
      healthEventsLoadFailed: '健康事件加载失败',
      healthEventAll: '全部事件',
      healthEventTypes: {
        success: '成功',
        failure: '失败',
