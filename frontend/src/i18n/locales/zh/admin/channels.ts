export default {
    ...
    channelMonitor: {
      ...
      advanced: {
        ...
        bodyMode: '请求体处理',
        responseMode: 'Responses 响应方式',
        responseModeJSON: '普通 JSON',
        responseModeSSE: '流式 SSE',
        responseModeHint: '按该监控的真实上游协议选择；流式模式会解析 Responses SSE 终止事件与文本增量。',
        bodyModeOff: '默认',
        bodyModeMerge: '合并',
        bodyModeReplace: '覆盖',
        bodyModeHintOff: '使用 adapter 默认请求体（带 challenge 数学题校验）。',
        bodyModeHintMerge: '与默认请求体浅合并，用户字段优先；但 model / messages / instructions / input / contents 会被保护不允许覆盖（动这些字段请用「覆盖」模式）。',
