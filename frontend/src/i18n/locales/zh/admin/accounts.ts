export default {
    accounts: {
      ...
      healthOverviewPanel: {
        ...
        total: '总计',
        schedulable: '有效可调度',
        isolatedDegraded: '隔离 / 降级',
        affectedGroups: '受影响分组',
        needsCoverage: '需要补充覆盖',
        keyRisks: '关键风险',
        risksMergedHint: '相同类型按上游、账号和分组去重合并，优先显示影响调度的风险。',
        levelCritical: '严重',
        levelWarning: '警告',
        levelInfo: '提示',
        locate: '定位',
        affectedAccounts: '影响 {count} 个账号',
        showAbnormal: '异常上游 {count}',
        showAll: '全部上游 {count}',
        allStates: '全部状态',
        anomalyFirst: '异常优先',
        searchPlaceholder: '搜索上游、账号、Key 或分组',
        expand: '展开上游账号',
        collapse: '折叠上游账号',
        upstreamHealthy: '当前上游未发现调度风险',
        noMatches: '没有符合当前筛选条件的上游或账号',
        riskAggregated: {
          urlAllUnavailable: '{count} 个上游全部不可用',
          groupNoneAvailable: '{count} 个分组没有可用账号',
          groupSingleAvailable: '{count} 个分组仅剩 1 个可用账号',
          consecutiveFailures: '{count} 个账号连续请求失败',
          probeDisabled: '{count} 个账号未启用健康巡检'
        }
      },
      riskSummary: '风险摘要',
      healthAccountCount: '{count} 个账号',
      healthInsufficientGroups: '分组可用账号不足：{groups}',
      healthDetail: '账号健康详情',
      healthDetailLoadFailed: '加载账号健康详情失败',
      healthScore: '健康分',
      boundGroups: '绑定分组',
      recentHealth: '近期健康',
      lastSuccess: '最近成功',
      ...
      lastError: '最近错误',
      probeSettings: '探活设置',
      probeSettingsHint: '控制异常恢复探活和健康账号低频巡检。',
      healthProbe: '手动探活',
      probeEnabled: '启用自动恢复探活',
      recoveryPriority: '恢复探活间隔',
      recoveryPriorityDefault: '默认',
      recoveryPriorityFast: '快速（3 分钟）',
      recoveryPriorityStandard: '标准（5 分钟）',
      recoveryPriorityConservative: '保守（10 分钟）',
      healthProbeModel: '探活模型',
      healthProbeModelAuto: '自动选择',
      healthProbeSyncModels: '同步模型',
      healthProbeModelHint: '用于健康探活的模型，留空时系统自动选择。',
      healthProbeSyncModelsSuccess: '模型同步成功',
      healthProbeSyncModelsFailed: '模型同步失败',
      probeIntervalCustom: '异常探活间隔（分钟）',
      probeIntervalPlaceholder: '例如 1、5、10',
      healthyProbeEnabled: '启用健康账号低频巡检',
      healthyProbeHint: '健康账号也会按配置周期自动探活，便于更新延迟和健康状态。',
      healthyProbeInterval: '健康巡检间隔',
      healthyProbeIntervalHint: '1 分钟会明显增加上游探活请求，建议仅用于测试或少量核心账号。',
      healthyProbeIntervalOptionHours: '{hours} 小时',
      healthyProbeIntervalOptionMinutes: '{minutes} 分钟',
      probeIntervalInvalid: '异常探活间隔必须大于 0',
      healthyProbeIntervalInvalid: '健康巡检间隔必须大于 0',
      probeSettingsSaved: '探活设置已保存',
      probeSettingsSaveFailed: '保存探活设置失败',
      healthProbeSuccess: '探活成功',
      healthProbeFailed: '探活失败',
      healthEvents: '健康事件',
      healthEventsHint: '查看账号健康状态变化和探活记录。',
      healthEventAll: '全部事件',
      healthEventsEmpty: '暂无健康事件',
      healthEventsLoadFailed: '加载健康事件失败',
      eventColumns: {
        time: '时间',
        type: '类型',
        source: '来源',
      ...
      upstreamBalanceRefresh: '刷新余额',
      upstreamBalanceRefreshSuccess: '余额刷新成功',
      upstreamBalanceRefreshFailed: '余额刷新失败',
      upstreamBalanceUnitUpstream: '上游额度 {value}',
      upstreamBalanceEmpty: '未查询',
      upstreamBalanceChecking: '查询中',
      upstreamBalanceValue: '余额 {value}',
      upstreamBalanceAuthError: '鉴权失败',
      upstreamBalanceError: '查询失败',
      upstreamBalanceUnsupported: '余额不可查',
      upstreamBalanceRepresentative: '代表账号 ID：{id}',
      upstreamBalanceCheckedAt: '更新时间：{time}',
      upstreamBalanceEndpoint: '查询接口：{endpoint}',
      upstreamBalanceUnitHint: '单位按上游站点展示配置',
      upstreamBalanceHTTPStatus: 'HTTP 状态：{status}',
      riskGroupNoAvailable: '分组无可用账号：{group}',
      riskGroupSingleAvailable: '分组仅 1 个可用账号：{group}',
      riskUrlAllIsolated: '该上游有 {count} 个账号全部隔离',
      riskConsecutiveFailures: '账号 {account} 连续失败 {count} 次',
      riskHealthyProbeDisabled: '账号 {account} 未开启健康巡检',
      healthStatus: {
        healthy: '健康',
        degraded: '降级',
        isolated: '隔离',
      ...
      upstreamBilling: {
        ...
        rechargeRatio: '上游充值比例',
        rechargeRatioHint: '输入 10 表示充值 1 元可获得 10 元上游余额；本站倍率按上游声明倍率除以该比例折算。',
        invalidRechargeRatio: '上游充值比例必须大于 0，保存时最多保留 4 位小数。',
        ...
        declaredRate: '上游声明倍率：{value}x',
        convertedRate: '本站折算倍率：{value}x',
        rechargeRatioValue: '上游充值比例：1:{value}',
      ...
      cnProviders: {
        ...
        skipBalanceCheck: {
          title: '跳过自动余额检查',
          hint: '仅适用于未实现 DeepSeek 官方余额接口的第三方上游。开启后不再周期查询余额；真实请求返回余额不足时仍会触发临时停调。',
        },
        balance: '余额 --',
        window5h: '5 小时窗口',
        windowWeekly: '每周窗口',
