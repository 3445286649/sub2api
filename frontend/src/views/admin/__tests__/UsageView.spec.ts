...
describe('admin UsageView model audit export', () => {
	...
	it('exports requested, sent, response, and mismatch as separate admin columns', async () => {
		...
		const tokenSpeedIndex = headers.indexOf('Token speed (Token/s)')
		expect(tokenSpeedIndex).toBeGreaterThan(-1)
		expect(row[tokenSpeedIndex]).toBe(100)
