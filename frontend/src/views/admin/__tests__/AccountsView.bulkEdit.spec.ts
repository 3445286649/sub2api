...
describe('admin AccountsView bulk edit scope', () => {
  ...
  it('refreshes the current page after a batch probe and displays the synced rate', async () => {
    ...
    expect(wrapper.get<HTMLInputElement>('[data-test="account-rate"] input').element.value).toBe('0.065')
    expect(wrapper.get('[data-test="account-rate"] .account-rate-suffix').text()).toBe('x')
  })
